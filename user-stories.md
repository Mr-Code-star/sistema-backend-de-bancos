...
| **US-IAM-06** | API RESTful – Verificar MFA                        | Como usuario quiero verificar mi método MFA para activarlo.                                  | **Escenario: Verificación MFA**.<br>**Cuando** envío `POST /api/v1/iam/auth/mfa/auth-app/verify` con<br>`json`<br>{<br> "methodId": "uuid",<br> "code": "123456"<br>}<br>**Entonces** el sistema valida el código y devuelve `200 OK`.                                                                                                                                                                                     | EP-01 |
